...
function getFilteredWallets(minLength = 3, typeFilter = null, searchInput = '', maxResults = Infinity, minSideLength = 2, includePrivateKeys = false, exportedFilter = 'all', nameStyleFilter = 'all', archivedFilter = 'not-archived', includeMeta = false, offset = 0, countTotal = true) {
    ...
    if (exportedFilter === 'exported') {
        ...
        for (const data of exportedRecords) {
            ...
            if (!countTotal && filtered.length >= limit) break;
        }
        const payload = includeMeta
            ? { wallets: filtered, totalMatches: countTotal ? totalMatches : null }
            : filtered;
        addToBoundedCache(filteredWalletsCache, cacheKey, payload, SEARCH_RESULT_CACHE_MAX);
        return payload;
    }
        ...
        try {
            forEachJsonlLineSync(jsonlPath, (line) => {
                try {
                    ...
                    if (matchesSearch) {
                        totalMatches += 1;
                        if (totalMatches <= skip) return true;
                        if (filtered.length < limit) {
                            filtered.push({
                                publicKey: data.publicKey,
                                privateKey: null,
                        ...
                        if (!countTotal && filtered.length >= limit) {
                            reachedLimit = true;
                            return false;
                        }
                    }
